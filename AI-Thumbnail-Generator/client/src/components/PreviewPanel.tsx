...
const PreviewPanel = ({thumbnail, isLoading, aspectRatio}:{thumbnail: IThumbnail | null , isLoading: boolean | null ; aspectRatio: AspectRatio }) => {
    ...
    const onDownload = () =>{
        ...
        window.open(thumbnail.image_url, 'blank')
    }
    ...
    <div className="relative mx-auto w-full max-w-2xl">
        ...
        {!isLoading && thumbnail?.image_url && (
            <div className="group relative h-full w-full">
                ...
                <div className="absolte inset-0 flex items-end justify-center bg-black/10 opacity-0 transition-opacity group-hover:opacity-100">
                <button onClick={onDownload} type="button" className=" flex items-center gap-2 rounded-md px-5 py-2.5 text-xs font-medium transition bg-white/30 ring-2 ring-white/40 backdrop-blur hover:scale-105 active:scale-95 -mt-10">Download Thumbnail <DownloadIcon className="size-4" /></button> 
                </div>
            </div>
        )}
